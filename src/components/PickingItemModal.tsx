import { Modal, Input, Button, Steps, StepProps, Form, Select, InputNumber } from "antd";
import Dayjs from "dayjs";
import { useEffect, useRef, useState } from "react";
import { PickingItem } from "./PickingDrawer";
...
const PickingItemModal: React.FC<PickingItemModalProps> = ({
    ...
    return (
        <Modal
            ...
            width={'60vw'}
        >
            <Steps current={current} items={items} type="navigation" size="small" className="mb-3" />
            ...
            {current === 0 ? (
                ...
                />) : null}
            {current === 1 ? (
                <Inbound selectedItem={selectedItem} setCurrent={setCurrent} handleClose={handleClose} />
            ) : null}
        </Modal>
    );
};
...
const SplitOrder = ({ selectedItem, setCurrent, handleClose }) => {
    ...
    const [current, total] = selectedItem?.quantity?.split(' / ').map(num => parseInt(num, 10)) || [0, 0];
            ...
            <div className="grid grid-cols-2 gap-4">
                <div className="col-span-2 grid grid-cols-2 gap-4 mb-4">
                    <div className="bg-blue-50 p-3 rounded-md border border-blue-100">
                        ...
                        <p className="text-2xl font-bold text-blue-700 tracking-wider">{selectedItem?.order}</p>
                    </div>
                    <div className="bg-blue-50 p-3 rounded-md border border-blue-100">
                        <p className="text-sm text-blue-600 font-medium">SKU</p>
                        <p className="text-2xl font-bold text-blue-700 tracking-wider">{selectedItem?.sku}</p>
                    </div>
                </div>
                <div>
                    <p className="text-sm text-gray-500">Conveyor Time</p>
                    <p className="font-medium">{Dayjs(selectedItem?.conveyor).format('HH:mm:ss')}</p>
                </div>
                <div>
                    <p className="text-sm text-gray-500">Order Time</p>
                    <p className="font-medium">{Dayjs(selectedItem?.ordered).format('HH:mm:ss')}</p>
                </div>
            </div>
            <div className="mt-6 p-4 bg-white rounded-lg shadow-sm">
                <div className="text-center">
                    ...
                    <div className="flex justify-center mt-4 gap-3">
                        <Button onClick={() => handleClose()} type="default">Cancel</Button>
                        <Button onClick={() =>
                            setCurrent(1)} type="primary">Next</Button>
                    </div>
                </div>
            </div>
        </div>
...
const Inbound = ({ selectedItem, setCurrent, handleClose }) => {
    ...
    return (
        <div>
            <div className="space-y-4 mt-5 bg-gray-50 p-4 rounded-lg">                <Form
                form={form}
                layout="vertical"
                ...
                <div className=" p-4 bg-white rounded-lg shadow-sm">
                    <div className="text-center">
                        ...
                        <Input
                            ref={refAction}
                            placeholder="Enter picking quantity or next action"
                            autoFocus className="text-center"
                            value={value}
                            onChange={handleAction}
                        />
                        <div className="flex justify-center mt-4 gap-3">
                            <Button onClick={() => setCurrent(0)} type="default">Cancel</Button>
                            <Button onClick={() => handleClose()} type="primary">Next</Button>
                        </div>
                    </div>
                </div>
            </div>
