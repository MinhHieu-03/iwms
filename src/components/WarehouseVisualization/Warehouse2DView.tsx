...
const Warehouse2DView: React.FC<Warehouse2DViewProps> = ({
  ...
  const { t } = useTranslation();
  const rackGroups_ = groupRacksByIdentifier(racks);
  const [rackGroups, setRackGroups] = React.useState(rackGroups_);
  useEffect(() => {
    apiClient
      .get('/area')
      .then(({ data }) => {
        console.log('Fetched Area Data:', data.metaData);
        const convertData = {};
        data.metaData.map((item: WarehouseArea) => {
          const { row, column, location_code } = item;
          const list = [];
          for (let i = 1; i <= row; i++) {
            for (let j = 1; j <= column; j++) {
              const locationCode = `${item.location_code}${i.toString().padStart(2, '0')}-${j.toString().padStart(2, '0')}`;
              list.push({
                area: 'A',
                areaId: locationCode,
                capacity: row * column,
          ...
          console.log('Converted Rack Groups:', convertData);
        });
      })
      .catch((error) => {
      ...
      <Card>
        <CardHeader className='pb-3'>
          ...
          {/* <div className='text-sm'>
            Utilization: {area.currentUtilization}/{area.capacity} (
            {Math.round((area.currentUtilization / area.capacity) * 100)}%)
          </div> */}
        </CardHeader>
        <CardContent>
          {/* Legend */}
